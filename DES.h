...
char* stringToBinary(char* s) {
    ...
    for(size_t j = len_binary; j < 64; ++j) {
    	strcat(binary,"0");
    }
...
char* BinaryToString(char* s) {
	//char *data = "01010110";
	int len = strlen(s),num = 0,num_char =0;
	char temp_c;
	char *tem_result = (char *)malloc((strlen(s)/8) + 1);
	char *data = (char *)malloc(8 + 1);
	tem_result[0] = '\0';
	...
	for(int i = 0; i < len; ++i) {
		if ( (i % 8 == 0) ){
			temp_c = strtol(data, 0, 2);
			if (i != 0){
				tem_result[num_char] = temp_c;
				num_char++;
...
string Desencriptar_clave(string ct){
   ...
   pt = ct;
	decrypted = DES();
    p_final = BinaryToString((char *)decrypted.c_str());
...
void menu(){
	system("cls");
	cout<<"\n    Inserccion de la clave.  "<<endl;
	cout<<"**************************************  "<<endl<<endl;
	cout<<" 1. Ingresar  la clave.                   "<<endl;
	cout<<" 2. Encriptar clave.                    "<<endl;
	cout<<" 3. Desencriptar clave.                 "<<endl;
	cout<<" 4. Salir.                              " <<endl;
...
int op = -1, x=0,i,j;
char clave[10];
string ct,decrypted, temp,p_final,pp;
	do{
		menu();
		cin>>op;
		switch (op){
			case 1: system("cls");
					...
					cout << "\n*******************************************" << endl;
               		...
               		cout << "********************************************" << endl<<endl;
					cout<<" Por favor Ingrese la clave debe ser 7 caracterer ";
					cin.getline(clave,10,'\n');
					if( strlen(clave)!=7){
						cout<<" Error  la clave debe tener 7 caracreres"<<endl;
					}
			...
			case 2: 
						...
						cin.ignore();
						cout << "\n************************************" << endl;
               			cout << "*         Clave incriptada.          *" << endl;
               			cout << "*************************************" << endl<<endl;
               			ct = Encriptar_clave(clave);
	            		 cout<< " La clave encriptada es:  " <<ct<<endl;
		        	break;
			case 3: system("cls");
					cout << "\n**********************************" << endl;
               		cout << "*        Desencriptar.            *" << endl;
               		cout << "**********************************" << endl<<endl;
               		 cout<< "  La clave desencriptada es: "<<Desencriptar_clave(ct)<<endl;
		        	break;
			case 4: system("cls");
					cout << "\n**********************************" << endl;
               		cout << "*     SAlir del menu clave.         *" << endl;
               		cout << "**********************************" << endl<<endl;
               		
					break;
	        default: cout<<"\n Marco una opcion incorrecta, intentelo de nuevo.  "<<endl;
	        		break;
    	}
		system("pause");
	}while(op!=4);
